...
# Assuming your updated PandorasLock script is in the same directory or properly installed as a module
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from PandorasLock.pandoraslockandkey import PandorasLock

# Use the default config path or specify if needed
config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'pandorasconfig.json'))

# Initialize PandorasLock with the correct config path
pandoras_lock = PandorasLock(config_path=config_path)  # Updated to use PandorasLock directly
...
# Sanitize the text
sanitized_text = pandoras_lock.sanitize(test_text)

# Print the sanitized text
print("Sanitized Text:\n", sanitized_text)

# Since the reverse_sanitization process might need adjustments for direct mapping, consider how it fits into your use case
